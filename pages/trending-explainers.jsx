...
const TickerTape = ({ items }) => {
  if (!items?.length) return null;
  const labelItem = { title: "Live Trending Index", isLabel: true };
          ...
          <span key={i} className="flex items-center gap-4 px-8 shrink-0">
            {item.isLabel ? (
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-white text-[11px] font-black uppercase tracking-[0.2em] whitespace-nowrap">Live Trending Index</span>
...
export default function TrendingExplainersPage() {
  ...
  // Fetch data from sync API based on region
  const fetchSyncData = async (region) => {
    try {
      setSyncing(true);
      console.log(`🔄 Fetching ${region === "IN" ? "Bollywood" : "Hollywood"} trends...`);
      
      const response = await fetch(`/api/trending/sync?region=${region}`); 
      const data = await response.json();
      
      if (data.success) {
        console.log(`✅ ${region === "IN" ? "Bollywood" : "Hollywood"} sync completed:`, data.stats);
        setLastSyncTime(new Date());
        return data;
      } else {
        console.error(`Failed to sync ${region}:`, data.message);
        return null;
      }
    } catch (error) {
  ...
  const fetchTrendingData = async (region) => {
    try {
      setLoading(true);
      ...
      if (data.success && data.data) {
        processTrendingData(data.data);
      } else {
        console.error("No data received from trending API");
        setTrendingData([]);
        setLoading(false);
      }
    } catch (error) {
  ...
  const processTrendingData = (rawData) => {
    ...
    if (rawData.trending_movies && Array.isArray(rawData.trending_movies)) {
      const movies = rawData.trending_movies
        ...
        .map(t => ({
          ...
          description: t.metadata?.overview || t.keywords?.slice(0, 3).join(" · ") || "Trending movie insight.",
          slug: t.slug,
    ...
    if (rawData.trending_actors && Array.isArray(rawData.trending_actors)) {
      const actors = rawData.trending_actors
        ...
        .map(t => ({
          ...
          description: t.metadata?.knownFor || t.keywords?.slice(0, 3).join(" · ") || "Trending celebrity.",
          slug: t.slug,
          source: t.source || "google",
          traffic: t.traffic || 0,
          viewCount: t.viewCount || 0,
    ...
    if (rawData.viral_topics && Array.isArray(rawData.viral_topics)) {
      const topics = rawData.viral_topics
        ...
        .map(t => ({
          ...
          description: t.keywords?.slice(0, 3).join(" · ") || "Trending topic.",
          slug: t.slug,
    ...
    const sorted = deduped.sort((a, b) => (b.trendScore || 0) - (a.trendScore || 0));
    
    setTrendingData(sorted);
    setCelebData(sorted.filter(x => x.category === "Celebrity"));
    setRawTrending({
      google: sorted.filter(x => x.source === "google"),
      youtube: sorted.filter(x => x.source === "youtube")
    });
    setLoading(false);
  };
  ...
  const handleRegionChange = async (region) => {
    ...
    setActiveRank(0);
    setActiveFilter("All");
    await fetchTrendingData(region);
  };
  ...
  return (
    <>
      <Head>
        <title>Trending Explainers — FilmyFire | {activeRegion === "IN" ? "Bollywood" : "Hollywood"} Trends</title>
        <meta name="description" content={`Latest ${activeRegion === "IN" ? "Bollywood" : "Hollywood"} trending movies, celebrity news, and entertainment intelligence.`} />
      </Head>
