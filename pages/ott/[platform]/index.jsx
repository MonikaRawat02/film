...
export async function getServerSideProps(context) {
  ...
  // List of known OTT platform slugs
  const knownPlatformSlugs = [
    'netflix', 'amazon-prime', 'amazon-prime-video', 'disney-plus-hotstar',
    'jiocinema', 'sonyliv', 'zee5', 'streaming' // 'streaming' is a fallback platform
  ];

  // If the 'platform' parameter is NOT a known platform slug,
  // it means it's likely a movie slug that was incorrectly routed here.
  // In this case, redirect to the correct movie detail page with a fallback platform.
  if (!knownPlatformSlugs.includes(platform.toLowerCase())) {
    return {
      redirect: {
        destination: `/ott/streaming/${platform}`, // Redirect to the correct dynamic route
