...
export async function generateMovieContent(movieData, pageType = "overview") {
  ...
  try {
    ...
    if (genAI) {
      try {
        console.log(`🚀 Using Google Gemini 2.5 Flash (PAID) for ${movieTitle} (${pageType})...`);
        ...
        console.error(`❌ Google Gemini 2.5 Flash failed: ${geminiErr.message}`);
      }
    }
...
export async function generateCelebrityData(celebName, industry = "Bollywood") {
  ...
  try {
    ...
    if (content) {
      ...
      cleanedJson = cleanedJson.replace(/(?<!\\)\n/g, '\\n').replace(/,\s*([\]}])/g, '$1').replace(/"([^"\\]*(\\.[^"\\]*)*)"/g, (match, p1) => {
          return '"' + p1.replace(/"/g, '\\"') + '"';
        });
      ...
      try {
        ...
        console.error(`❌ JSON Parse Error for ${celebName}: ${parseError.message}`);
