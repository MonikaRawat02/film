...
import { validateContentQuality } from "./content-validator";
...
export async function generateMovieContent(movieData, pageType = "overview") {
  ...
  const prompts = {
    overview: `Generate a comprehensive, SEO-optimized article of 1800-2200 words for the movie "${movieTitle}" ${yearStr}. This is the main pillar page and must be exceptionally detailed.
      ...
      **Article Structure:**
      ...
      # ${movieTitle} Movie ${yearStr} – Full Analysis, Box Office & OTT Details
    ...
    "ending-explained": `Generate a deep-dive "Ending Explained" article of 1200-1500 words for "${movieTitle}" ${yearStr}. The focus is on analysis, symbolism, and thematic interpretation.
    ...
    "box-office": `Generate a detailed financial analysis of 1200-1500 words for "${movieTitle}" ${yearStr}. The article must be data-driven and analytical.
      ...
      **SPECIAL INSTRUCTION (JSON EXTRACTION):**
      At the very end of your response, after the article, provide a JSON block with the following structure containing realistic financial estimates for 2025:
      
      JSON_DATA_START
      {
        "verdict": "Blockbuster | Super Hit | Hit | Average | Flop",
        "territorialBreakdown": [
          { "region": "Mumbai Circuit", "percentage": 25, "amount": "₹150 Cr" },
          { "region": "Delhi/UP", "percentage": 20, "amount": "₹120 Cr" },
          { "region": "East Punjab", "percentage": 15, "amount": "₹90 Cr" },
          { "region": "Nizam/AP", "percentage": 18, "amount": "₹108 Cr" }
        ],
        "overseasMarkets": [
          { "market": "UAE/GCC", "amount": "$15M" },
          { "market": "USA/Canada", "amount": "$20M" },
          { "market": "UK/Europe", "amount": "$8M" }
        ]
      }
      JSON_DATA_END`,
  ...
  try {
    ...
    // 1. Try AI Generation with Fallback (Flash/Pro)
    try {
      console.log(`🚀 Generating content for ${movieTitle} (${pageType}) with fallback...`);
      ...
      console.error(`❌ AI Content Generation failed: ${geminiErr.message}`);
    }
    ...
    // --- NEW: JSON Extraction for Box Office ---
    let extraData = null;
    if (pageType === "box-office") {
      try {
        const jsonMatch = content.match(/JSON_DATA_START([\s\S]*?)JSON_DATA_END/);
        if (jsonMatch) {
          extraData = JSON.parse(jsonMatch[1].trim());
          // Remove JSON from displayed content
          content = content.replace(/JSON_DATA_START[\s\S]*?JSON_DATA_END/, "").trim();
        }
      } catch (jsonErr) {
        console.error("❌ Failed to parse box office JSON:", jsonErr.message);
      }
    }
    ...
    console.log(`📊 Validating content quality for ${movieTitle}...`);
    const validationReport = await validateContentQuality(sections, movieData, {
      minWords: pageType === "overview" ? 1800 : 1200,
      maxWords: 2500,
      targetKeywords: [movieTitle, movieData.releaseYear, ...(movieData.genres || [])].filter(Boolean),
      requireFAQ: true,
    ...
    return { 
      ...
      extraData // Pass extracted JSON data
    };
  } catch (error) {
...
/**
 * Regenerate expanded content when word count is too low
 */
async function regenerateExpandedContent(movieData, pageType, existingSections) {
  const { movieTitle, releaseYear } = movieData;
  
  const expansionPrompt = `Expand and improve the following content for "${movieTitle}" (${releaseYear}). 
  ...
  Output the complete expanded article with proper H2/H3 headings.`;
  ...
  try {
    ...
    console.error(`Regeneration error for ${movieTitle}:`, error.message);
    return null;
  }
}

/**
 * Generates a minimal celebrity data structure as fallback
 */
function createMinimalCelebrityData(celebName, industry) {
  console.log(`⚠️ Creating minimal fallback data for ${celebName}`);
  
  return {
    heroSection: {
      profession: ["Actor", "Performer"],
      careerStage: "Active",
      nationality: industry === "Bollywood" ? "Indian" : "American",
      height: "5'10\"",
      awardsCount: 0,
      growthPercentage: 0
    },
    netWorth: {
      ...
      year: 2025,
      description: `Net worth details for ${celebName} are being updated.`,
      netWorthINR: { min: 0, max: 0, display: "₹0 Cr" },
      netWorthUSD: { min: 0, max: 0, display: "$0 M" },
      analysisSummary: "Data pending verification."
    },
    quickFacts: {
      activeSince: new Date().getFullYear() - 5,
      brandEndorsements: 0
    },
    netWorthCalculation: {
      incomeSources: [
        { sourceName: "Film Industry", percentage: 100, description: "Primary income source" }
      ]
    },
    netWorthTimeline: {
      timeline: [],
    ...
    biographyTimeline: [
      { period: "Early Life", title: "Born in...", description: `${celebName} is a recognized figure in the ${industry} industry.` }
    ],
    assets: []
  };
}
  ...
  try {
    ...
    if (content) {
      ...
      if (firstBrace === -1 || lastBrace === -1) {
        console.error(`❌ No valid JSON structure found in response for ${celebName}`);
        return null;
      }
      ...
      try {
        ...
        console.log(`✅ Successfully generated celebrity data for ${celebName}`);
        return parsedData;
      } catch (firstParseError) {
        console.warn(`⚠️ Initial JSON parse failed for ${celebName}, attempting cleanup...`);
        
        // Only if direct parse fails, try basic cleanup
        let attemptCleanup = cleanedJson
          .replace(/,\s*([\]}])/g, '$1') // Remove trailing commas
          .replace(/(?<!\\)\n/g, ' ')    // Replace unescaped newlines with spaces
          .trim();
          
        try {
          const secondParsedData = JSON.parse(attemptCleanup);
          console.log(`✅ Successfully parsed celebrity data after cleanup for ${celebName}`);
          return secondParsedData;
        } catch (secondParseError) {
          console.error(`❌ JSON Parse Error for ${celebName}: ${secondParseError.message}`);
          console.error("Mangled JSON was:", attemptCleanup.substring(0, 100) + "...");
          return createMinimalCelebrityData(celebName, industry);
        }
      }
    }
    return null;
