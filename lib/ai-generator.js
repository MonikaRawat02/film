  ...
  try {
    ...
    if (genAI) {
      try {
        ...
        // Using Gemini 2.5 Flash - latest stable model for production use
    ...
    const validationReport = await validateContentQuality(sections, movieData, {
      ...
      checkDuplicates: false // Set to true and pass existingArticles for duplicate check
    });

    console.log(`📊 Quality Score: ${validationReport?.qualityScore || 0}/100`);
    
    if (!validationReport || !validationReport.isValid) {
      console.warn(`⚠️ Content quality below threshold (${validationReport?.qualityScore || 0}%). Issues:`, validationReport?.issues || ['Validation failed']);
      
      // Attempt to improve content by requesting AI regeneration with specific instructions
      if (validationReport?.wordCount && validationReport.wordCount < 1200) {
        console.log(`🔄 Regenerating expanded content for ${movieTitle} (current: ${validationReport.wordCount} words)...`);
        const improvedContent = await regenerateExpandedContent(movieData, pageType, sections);
        if (improvedContent) {
          return { sections: improvedContent, isAI: true };
        }
      }
    }

    if (validationReport?.recommendations && validationReport.recommendations.length > 0) {
      console.log(`💡 Recommendations for ${movieTitle}:`, validationReport.recommendations);
    }

    return { sections, isAI: true, qualityReport: validationReport };
  } catch (error) {
...
function createMinimalCelebrityData(celebName, industry) {
  ...
  return {
    ...
    biographyTimeline: [
      { period: "Career", title: "Professional Journey", description: `${celebName} is a recognized figure in the ${industry} industry.` }
    ],
    assets: []
  };
}
  ...
  try {
    ...
    if (content) {
      ...
      try {
        ...
        console.error(`Raw response length: ${content.length} chars`);
