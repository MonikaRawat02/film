...
export async function autoLinkMovies(content, options = {}) {
  ...
  for (const [key, movie] of sortedMovies) {
    ...
    processedContent = processedContent.replace(titlePattern, (match) => {
      ...
      return `<a href="/movie/${movie.slug}" class="text-red-400 hover:text-red-300 underline decoration-red-500/30 hover:decoration-red-400 transition-colors font-medium">${match}</a>`;
    });
  }
