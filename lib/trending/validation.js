...
export async function validateMovie(title, keywords) {
  try {
    ...
    if (!coreTitle || coreTitle.length < 3) return { isValid: false };
    ...
    if (TMDB_API_KEY) {
      ...
      if (tmdbMovie) {
        console.log(`      🌐 TMDB Search: Found "${tmdbMovie.title}" (Popularity: ${tmdbMovie.popularity}, Votes: ${tmdbMovie.vote_count})`);
        
        // Check if it's a real movie (popularity > 5 or significant votes)
        if (tmdbMovie.popularity > 5 || tmdbMovie.vote_count > 10) {
          console.log(`      ✅ TMDB MATCH: ${tmdbMovie.title}`);
          return {
            isValid: true,
            type: "trending_movies",
            entityType: "movie",
        ...
        console.log(`      🌐 TMDB Search: No results found`);
      }
    }
...
export async function validateActor(title, keywords) {
  try {
    ...
    if (TMDB_API_KEY) {
      ...
      if (tmdbPerson && tmdbPerson.popularity > 1) {
        console.log(`      ✅ TMDB MATCH: ${tmdbPerson.name}`);
        return {
          isValid: true,
          type: "trending_actors",
          entityType: "actor",
