...
export async function preprocessTrend(trend) {
  ...
  // Use AI for better accuracy if title is substantial and we have OPENAI_API_KEY
  let entityType, confidence, finalKeywords;

  if (openAIConfigured && trend.title.split(' ').length >= 3) {
    try {
...
Respond ONLY with valid JSON:
{
  ...
  if (!text) throw new Error("OpenAI model failed");
