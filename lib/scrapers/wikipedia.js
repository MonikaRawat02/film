...
function extractNetWorth(text, industry = "Hollywood") {
  ...
  for (const pattern of Object.values(patterns)) {
    ...
    if (match) {
      ...
      let maxVal = range[1] ? parseFloat(range[1]) : minVal;
      const unit = match[2].toLowerCase();
      
      // Validate values - reject anything less than 1 million
...
export async function scrapeWikipediaCelebrity(url, industry = "Bollywood") {
  try {
    ...
    if (netWorthText) {
      ...
      // Only use extracted values if they're valid (at least 1M USD or equivalent)
      const hasValidUSD = usdValue && usdValue.min >= 1;
    ...
    const needsEstimate = 
      ...
      (celebrityData.netWorth.netWorthUSD.min && celebrityData.netWorth.netWorthUSD.min < 1);
    ...
    if (needsEstimate) {
      ...
      const estimatedMin = Math.max(Math.round(totalFilms * basePerFilm * (1 + experienceBonus * 0.5)), 5); // Minimum $5M
