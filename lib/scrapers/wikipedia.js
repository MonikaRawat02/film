...
import { fetchCelebrityFromTMDB } from "../api-clients/tmdb.js";
...
function extractNetWorth(text, industry = "Hollywood") {
  const patterns = [
    // Match: $1.2 billion, US$800 million, USD 1.5B
    /(?:US\$|USD|\$)\s*(\d+(?:\.\d+)?)\s*(billion|B)/i,
    /(?:US\$|USD|\$)\s*(\d+(?:\.\d+)?)\s*(million|M)/i,
    // Match: net worth of $1.2 billion
    /net\s*worth\s*(?:of|is|estimated\s*(?:at|to|be))?\s*(?:around\s*)?(?:US\$|USD|\$)?\s*(\d+(?:\.\d+)?)\s*(billion|B)/i,
    /net\s*worth\s*(?:of|is|estimated\s*(?:at|to|be))?\s*(?:around\s*)?(?:US\$|USD|\$)?\s*(\d+(?:\.\d+)?)\s*(million|M)/i,
    // Match Indian Rupees: ₹5,000 crore, Rs. 6000 Cr
    /(?:₹|Rs\.?|INR)\s*(\d+(?:,\d+)*(?:\.\d+)?)\s*(crore|Cr|billion|B)/i,
  ];
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      // Validate values
      if (isNaN(minVal) || minVal < 0.1) {
        continue;
      }

      // Convert to millions for standardization
      let valueInMillions;
      if (unit.startsWith('b')) {
        // Billion to Million
        valueInMillions = minVal * 1000;
      } else if (unit.includes('crore') || unit === 'cr') {
        // Crore to Million (1 crore = 10 million)
        valueInMillions = minVal * 10;
        // This is INR, store separately
        inrValue = { 
          min: minVal, 
          max: minVal,
          unit: 'crore'
  ...
  // If we found INR but not USD, convert
  if (inrValue && !usdValue) {
    const inrInCrores = inrValue.min;
    const usdInMillions = (inrInCrores * 10) / LIVE_USD_TO_INR_RATE;
    usdValue = {
      min: usdInMillions,
      max: usdInMillions,
      originalUnit: 'million'
    };
  }

  // If we found USD but not INR, convert
  if (usdValue && !inrValue) {
    const usdInMillions = usdValue.min;
    const inrInCrores = (usdInMillions * LIVE_USD_TO_INR_RATE) / 10;
    inrValue = {
      min: inrInCrores,
      max: inrInCrores,
      unit: 'crore'
...
export async function scrapeWikipediaCelebrity(url, industry = "Bollywood") {
  try {
    ...
    // Extract profile image - try Wikipedia first, then TMDB for valid image
    let profileImageFound = false;
    const imageEl = infoBox.find("img").first();
    if (imageEl.length) {
      let src = imageEl.attr("src");
      if (src) {
        // Get high-res version by removing thumbnail scaling (like we do for movies)
        if (src.includes("/thumb/")) {
          const parts = src.split("/");
          parts.pop();
          parts.splice(parts.indexOf("thumb"), 1);
        ...
        src = src.startsWith("http") ? src : `https:${src}`;
        celebrityData.heroSection.profileImage = src;
        profileImageFound = true;
      }
    }

    // Try TMDB for valid profile image if Wikipedia didn't give us a good one
    if (!profileImageFound || !celebrityData.heroSection.profileImage || celebrityData.heroSection.profileImage.includes("placeholder")) {
      try {
        const tmdbData = await fetchCelebrityFromTMDB(name);
        if (tmdbData && tmdbData.profileImage) {
          celebrityData.heroSection.profileImage = tmdbData.profileImage;
          // Also add TMDB ID for future reference
          celebrityData.heroSection.tmdbId = tmdbData.tmdbId;
        }
      } catch (tmdbErr) {
        console.log(`⚠️ TMDB image fetch failed for ${name}:`, tmdbErr.message);
        // Continue with Wikipedia data if TMDB fails
      }
    }
      ...
      switch (true) {
        ...
        case label.includes("born"):
          ...
          if (dateMatch) {
            ...
            // Don't calculate age here - will be calculated after death date check
          }
          const ageMatch = value.match(/age\s*(\d+)/i);
          if (ageMatch) celebrityData.quickFacts.age = parseInt(ageMatch[1]);
          break;
        case label.includes("died") || label.includes("death"):
          const deathDateMatch = value.match(/(\d{1,2}\s+\w+\s+\d{4})|(\w+\s+\d{1,2},\s+\d{4})/);
          if (deathDateMatch) {
            const deathDate = new Date(deathDateMatch[0]);
            celebrityData.quickFacts.deathDate = deathDate;
            celebrityData.heroSection.isDeceased = true;
            celebrityData.quickFacts.isDeceased = true;
            
            // Calculate age at death
            if (celebrityData.quickFacts.birthDate) {
              const birthYear = new Date(celebrityData.quickFacts.birthDate).getFullYear();
              const deathYear = deathDate.getFullYear();
              celebrityData.quickFacts.age = deathYear - birthYear;
            }
          }
          break;
    ...
    // Calculate age for living celebrities if not already set
    if (!celebrityData.quickFacts.isDeceased && celebrityData.quickFacts.birthDate && !celebrityData.quickFacts.age) {
      const birthYear = new Date(celebrityData.quickFacts.birthDate).getFullYear();
      const currentYear = new Date().getFullYear();
      celebrityData.quickFacts.age = currentYear - birthYear;
    }
    ...
    // Count films from filmography section
    let filmsCounted = false;
    $("h2, h3").each((_, heading) => {
      const $h = $(heading);
      const title = cleanText($h.text().replace(/\[edit\]/g, "")).toLowerCase();
      
      if ((title.includes('filmography') || title.includes('films') || title.includes('television')) && !filmsCounted) {
        filmsCounted = true;
        let curr = $h.next();
        let maxDepth = 20; // Prevent infinite loops
        let depth = 0;
        
        while (curr.length && depth < maxDepth && !curr.is("h2")) {
          if (curr.is('table.wikitable, table.plainrowheaders')) {
            // Count rows in filmography table (excluding header)
            const rows = curr.find('tr').length;
            celebrityData.heroSection.filmsCount += Math.max(0, rows - 1);
          } else if (curr.is('ul')) {
            // Count list items
      ...
      // Count awards and nominations
      if (title.includes('awards and nominations') || title.includes('awards') || title.includes('accolades')) {
        let curr = $h.next();
        let maxDepth = 20;
        let depth = 0;
        let awardsCount = 0;
        ...
        while (curr.length && depth < maxDepth && !curr.is("h2")) {
          if (curr.is('table.wikitable, table.plainrowheaders')) {
            // Each row represents an award/nomination
            const rows = curr.find('tr').length;
            awardsCount += Math.max(0, rows - 1);
          } else if (curr.is('ul')) {
            awardsCount += curr.find('li').length;
          }
          curr = curr.next();
          depth++;
        }
        
        // Set awards count (typically 30-50% of nominations are wins)
        celebrityData.heroSection.awardsCount = Math.floor(awardsCount * 0.4);
      }
    });
    
    // If filmography wasn't found in sections, try to find it in the infobox or references
    if (celebrityData.heroSection.filmsCount === 0) {
      // Try to extract from "Years active" and estimate
      if (celebrityData.heroSection.activeSince) {
        const yearsActive = new Date().getFullYear() - celebrityData.heroSection.activeSince;
        // Estimate based on industry average
        celebrityData.heroSection.filmsCount = industry === "Bollywood" ? 
          Math.floor(yearsActive * 2.5) : Math.floor(yearsActive * 1.5);
      }
    }
    ...
    if (netWorthText) {
      ...
      // Only use extracted values if they're valid
      const hasValidUSD = usdValue && usdValue.min >= 0.5; // At least $500K
      const hasValidINR = inrValue && inrValue.min >= 1;
      
      if (hasValidUSD) {
        // Format display properly for millions vs billions
        let displayUSD;
        if (usdValue.min >= 1000) {
          // Billions
          const billions = (usdValue.min / 1000).toFixed(1);
          displayUSD = usdValue.min !== usdValue.max 
            ? `$${(usdValue.min / 1000).toFixed(1)}-${(usdValue.max / 1000).toFixed(1)}B`
            : `$${billions}B`;
        } else {
          // Millions
          displayUSD = usdValue.min !== usdValue.max
            ? `$${usdValue.min.toFixed(0)}-${usdValue.max.toFixed(0)}M`
            : `$${usdValue.min.toFixed(0)}M`;
        }
        ...
        celebrityData.netWorthAnalysis.displayRange = displayUSD;
      }
      
      if (hasValidINR) {
        // Format INR display
        let displayINR;
        if (inrValue.unit === 'crore') {
          displayINR = inrValue.min !== inrValue.max
            ? `₹${inrValue.min.toFixed(0)}-${inrValue.max.toFixed(0)} Cr`
            : `₹${inrValue.min.toFixed(0)} Cr`;
        } else {
          displayINR = inrValue.min !== inrValue.max
            ? `₹${inrValue.min.toFixed(0)}-${inrValue.max.toFixed(0)} Cr`
            : `₹${inrValue.min.toFixed(0)} Cr`;
    ...
    // Generate estimated net worth if none found OR if invalid values were extracted
    const needsEstimate = 
      celebrityData.netWorth.netWorthUSD.display === "N/A" || 
      (celebrityData.netWorth.netWorthUSD.min && celebrityData.netWorth.netWorthUSD.min < 0.5); // Less than $500K
      
    if (needsEstimate) {
      const yearsActive = celebrityData.heroSection.activeSince ? (new Date().getFullYear() - celebrityData.heroSection.activeSince) : 10;
      const totalFilms = Math.max(celebrityData.heroSection.filmsCount || 0, 10); // Ensure at least 10 films for estimate
      const fameMultiplier = celebrityData.heroSection.awardsCount > 20 ? 2.5 : celebrityData.heroSection.awardsCount > 10 ? 1.8 : 1;
      
      // More realistic estimation based on industry and fame
      let basePerFilm;
      if (industry === "Bollywood") {
        basePerFilm = totalFilms > 100 ? 2 : totalFilms > 50 ? 1.2 : 0.8; // Top actors earn more per film
      } else {
        basePerFilm = totalFilms > 50 ? 5 : totalFilms > 20 ? 3 : 2.5; // Hollywood pays more
      }
      
      const experienceBonus = Math.min(3, yearsActive / 15);
      
      const estimatedMin = Math.max(Math.round(totalFilms * basePerFilm * (1 + experienceBonus * 0.5) * fameMultiplier), 5); // Minimum $5M
      const estimatedMax = Math.max(Math.round(estimatedMin * 2.5), 10); // Minimum $10M max

      // Format display properly for millions vs billions
      let displayUSD;
      if (estimatedMin >= 1000) {
        // Billions
        displayUSD = estimatedMin !== estimatedMax
          ? `$${(estimatedMin / 1000).toFixed(1)}-${(estimatedMax / 1000).toFixed(1)}B`
          : `$${(estimatedMin / 1000).toFixed(1)}B`;
      } else {
        // Millions
        displayUSD = estimatedMin !== estimatedMax
          ? `$${estimatedMin}-${estimatedMax}M`
          : `$${estimatedMin}M`;
      }
      ...
      // Format INR
      let displayINR;
      const inrMin = Math.round((estimatedMin * LIVE_USD_TO_INR_RATE) / 10); // Convert to crores
      const inrMax = Math.round((estimatedMax * LIVE_USD_TO_INR_RATE) / 10);
      
      if (inrMin >= 1000) {
        displayINR = inrMin !== inrMax
          ? `₹${(inrMin / 1000).toFixed(1)}-${(inrMax / 1000).toFixed(1)}K Cr`
          : `₹${(inrMin / 1000).toFixed(1)}K Cr`;
      } else {
        displayINR = inrMin !== inrMax
          ? `₹${inrMin}-${inrMax} Cr`
          : `₹${inrMin} Cr`;
