 ...
 * Uses suffix-style URLs: /movie/:slug and /movie/:slug-suffix
 */
export function validateCanonicalURL(slug, pageType) {
  // For suffix-style, the slug IS the full URL slug (e.g., "animal-box-office")
  const baseSlug = slug.replace(/-ending-explained|-box-office|-budget|-ott-release|-cast|-review-analysis|-hit-or-flop$/, '');
  
  const expectedPatterns = [
    `/movie/${baseSlug}`,                         // Overview page
    `/movie/${baseSlug}-ending-explained`,        // Sub-pages (suffix-style)
  ...
  const currentURL = pageType === 'overview' ? `/movie/${baseSlug}` : `/movie/${baseSlug}-${pageType}`;
  const isValid = expectedPatterns.includes(currentURL);
