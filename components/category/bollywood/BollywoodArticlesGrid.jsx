  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((article) => (
            <Link
              key={article._id}
              href={`/bollywood/movies/${article.slug}`}
      ...
      <div className="mt-16 text-center">
        <div className="inline-block w-full max-w-4xl p-3 rounded-lg border border-dashed border-amber-500/30 bg-amber-500/5">
          <p className="text-[10px] font-bold text-amber-300/80 uppercase tracking-widest">
            Automated Updates: This section updates daily via API integration - SEO URL: /bollywood/movies/[movie-name]
          </p>
        </div>
      </div>
    </section>
